...
document.addEventListener('DOMContentLoaded', function () {
    ...
    setLanguage(currentLang); // Aplica o idioma salvo

    // Placeholder functions para garantir que a página não quebre se o JS/config.js externo falhar
    window.handleThemeChange = function (val) { console.log('Theme changed to:', val); };
    window.resetAppearance = function () { showToast(t('toast.appearanceSaved')); };
    ...
    window.toggleInvertColors = function () { document.body.classList.toggle('invert-colors'); };
    window.changeFontSize = function (val) {
        const labels = ['Pequeno', 'Normal', 'Grande', 'Muito Grande'];
        document.getElementById('fontSizeValue').textContent = labels[val - 1];
    };
    window.changeLineHeight = function (val) {
        const labels = ['Compacto', 'Normal', 'Espaçado'];
        document.getElementById('lineHeightValue').textContent = labels[val - 1];
    };
    window.toggleBigCursor = function () { document.body.classList.toggle('big-cursor'); };
    window.toggleHighlightLinks = function () { document.body.classList.toggle('highlight-links'); };
    window.toggleVisibleFocus = function () { document.body.classList.toggle('visible-focus'); };
    window.saveAccessibilitySettings = function () { showToast(t('btn.saveSettings')); };
    window.resetAccessibility = function () { showToast(t('btn.restoreDefaults')); };
    window.confirmDeleteAccount = function () { if (confirm(t('account.dangerZoneDesc'))) window.location.reload(); };
});
...
function exportUserData() {
    const data = { perfil: Storage.get('userProfile', {}), dataExportacao: new Date().toISOString() };
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('btnSalvarAparencia').addEventListener('click', () => {
        ...
        showToast('Preferências de aparência aplicadas!', 'success');
    });
    ...
    document.getElementById('btnSalvarAcessibilidade').addEventListener('click', () => {
        const settings = {
        ...
        showToast('Configurações de acessibilidade aplicadas!', 'success');
    });
    ...
    document.getElementById('btnSalvarNotificacoes').addEventListener('click', () => {
        showToast('Preferências de notificação aplicadas!', 'success');
    });

    // Privacidade → Aplicar
    document.getElementById('btnSalvarPrivacidade').addEventListener('click', () => {
        showToast('Configurações de privacidade aplicadas!', 'success');
    });
